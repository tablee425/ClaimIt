...
namespace ClaimIt.DayViewComponent
{
  ...
  public class DayView : AbsoluteLayout
  {
    /// <summary>
    /// CoverFlow items source property.
    /// </summary>
    public static BindableProperty ItemsSourceProperty = BindableProperty.Create(nameof(ItemsSource), typeof(IList), typeof(DayView), null, propertyChanged: (bindable, oldValue, newValue) =>
    {
      bindable.AsDayView().GenerateCoverList(bindable);
    });

    /// <summary>
    /// The item template property.
    /// </summary>
    public static readonly BindableProperty ItemTemplateProperty = BindableProperty.Create(nameof(ItemTemplate), typeof(DataTemplate), typeof(DayView), propertyChanged: (bindable, oldValue, newValue) =>
    {
      bindable.AsDayView().GenerateCoverList(bindable);
    });
    ...
    /// </summary>
    public static readonly BindableProperty NumberOfViewsProperty = BindableProperty.Create(nameof(NumberOfViews), typeof(int), typeof(DayView), 9, propertyChanged: (bindable, oldValue, newValue) =>
    {
    ...
    /// </summary>
    public static readonly BindableProperty SpacingProperty = BindableProperty.Create(nameof(Spacing), typeof(double), typeof(DayView), default(double));

    /// <summary>
    /// The Cyclical property.
    /// </summary>
    public static readonly BindableProperty IsCyclicalProperty = BindableProperty.Create(nameof(IsCyclical), typeof(bool), typeof(DayView), true);

    /// <summary>
    /// The First Item position property.
    /// </summary>
    public static readonly BindableProperty FirstItemPositionProperty = BindableProperty.Create(nameof(FirstItemPosition), typeof(DayPosition), typeof(DayView), DayPosition.Center);

    /// <summary>
    /// The view position property.
    /// </summary>
    public static readonly BindableProperty ViewAlignmentProperty = BindableProperty.Create(nameof(ViewAlignment), typeof(DayPosition), typeof(DayView), DayPosition.Center);

    private readonly PanGestureRecognizer _panGesture = new PanGestureRecognizer();
    private List<View> _recycledViews = new List<View>();
    private BaseDayViewProcessor ViewProcessor { get; }
    ...
    public DayView(BaseDayViewProcessor baseCoverFlowProcessor)
    {
    ...
    public DayView() : this(new BaseDayViewProcessor())
    {
    }
    ...
    public DayPosition FirstItemPosition
    {
      get => (DayPosition)GetValue(FirstItemPositionProperty);
    ...
    public DayPosition ViewAlignment
    {
      get => (DayPosition)GetValue(ViewAlignmentProperty);
    ...
    */
    ...
    {
      if (bindable is DayView CoverFlow && CoverFlow.ItemTemplate is DataTemplate itemTemplate && CoverFlow.ItemsSource is IList itemsSource)
      {
    ...
    private int GetIndexCloseToAlignment(DayPosition viewPosition)
    {
    ...
    private void OnDragEnd()
    {
      ...
      if (!IsCyclical && ViewAlignment != DayPosition.Center && _tmpTotalX < 0 && ItemMaxOnAxis == ItemsSource.Count - 1)
      {
        position = DayPosition.Right;
      }
      if (!IsCyclical && ViewAlignment != DayPosition.Center && _tmpTotalX > 0 && ItemMinOnAxis == 0)
      {
        position = DayPosition.Left;
      }
